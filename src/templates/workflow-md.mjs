...
export default function workflowMdTemplate(config) {
  const repoName = config.repoName;
  const baseBranch = config.baseBranch;
  const install = installCmd(config.packageManager);
  const cmd = runCmd(config.packageManager);

  return `# ${repoName} OSS Workflow

${repoName} uses a lightweight open-source maintenance workflow. The goal is to keep contributor work small, reviewable, and verifiable without training-camp scoring or task-claim automation.
...
-> ${cmd} quality:local before pushing
-> PR to ${baseBranch} with structured self-check
...
2. Run \`${cmd} agent:bootstrap\` and \`${cmd} quality:predev\` before code changes when feasible.
...
This repository does not use claim, score, progress ledgers, auto-merge comments, or PR timeout-close automation.
`;
}
