...
export default function harnessWorkflowTemplate(config) {
  ...
  return `# OSS Harness Engineering Workflow

This document is the detailed repository asset for the ${repoName} OSS Harness engineering workflow. It expands the lightweight loop in \`docs/workflow.md\` into an operational playbook that agents, maintainers, and contributors can follow for non-trivial repository work.

The workflow is adapted from the OSS Harness pattern, scoped to open-source community maintenance. It excludes training-camp mechanics such as task claiming, scoring, progress ledgers, timeout-close automation, and comment-triggered auto-merge.
...
Use a short-lived branch from \`${baseBranch}\`: \`feat/<topic>\`, \`fix/<topic>\`, \`docs/<topic>\`, or \`chore/<topic>\`.
...
Before editing critical skeleton paths, run \`${cmd} contract:check\` to classify the affected surfaces. For symbol-level impact, use GitNexus if available; otherwise perform manual impact analysis.
...
Open PRs to \`${baseBranch}\`. Link the issue, discussion, or explain why the PR stands alone. Fill the PR template with summary, impact scope, verification, and gate results. For critical skeleton changes, fill a concrete Impact Summary.
